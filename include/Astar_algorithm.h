...
void UpdataMapInfo(const int row_args, const int columns_args,
                   const Points& start_args,
                   const std::vector<Points>& map_obstacle_list,
...
// A*算法函数
std::vector<Points> Astar_Algorithm(const int row_, const int columns_,
                                    const Points& statr_, const Points& goal_,
                                    const std::vector<Points>& close_list_);
//计算当前点与终点距离的函数
